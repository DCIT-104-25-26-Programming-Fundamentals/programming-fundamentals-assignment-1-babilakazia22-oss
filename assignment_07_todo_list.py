...
def view_tasks():
    """Display all tasks in the list, numbered from 1."""
    if not tasks:
...
def delete_task():
    """Delete a task by its number, with validation."""
    if not tasks:
    ...
    try:
        ...
        index = choice - 1  # convert to zero-based index
...
def main():
    """Main loop that keeps the menu running until user quits."""
    while True:
        show_menu()
        choice = input()
